...
def _read_mx(mx_dir,patt):
    ...
    old_mx_patt = '白鹭郡|百汇|梅溪郡|滨江|缇香|紫台'
    com_old_mx = re.compile(old_mx_patt)
    file_dic = {}
...
def file_timestamp2date(dir_name,timestamp_handle=lambda x:x*1000000):
...
def update_vk_rep_path(dir_name,com_patt,format_date,extend_name = 'xlsx'):
    file_timestamp2date(dir_name)

    patt = com_patt
    com = re.compile(patt)
    date = pd.to_datetime('1900-1-1')
    file_name = None
    for p,d,fs_name in os.walk(dir_name):
...
def read_vanke_rep(rep_dir,com_patt,format_date,sheet_name,rep_name):
...
def read_lp(cwlp_dir, xm_lp_map_mx):
    com_patt = r'^财务楼盘表(?P<date>20\d{2}-\d{2}-\d{2} \d{2}_\d{2}_\d{2})\.xlsx$'
    format_date = '%Y-%m-%d %H_%M_%S'
    sheet_name = "财务楼盘表0"
    rep_name = "财务楼盘表"
    try :
        df_lp = read_vanke_rep(cwlp_dir,com_patt,format_date,sheet_name,rep_name)
    except:
        sheet_name = "财务楼盘表"
        df_lp = read_vanke_rep(cwlp_dir,com_patt,format_date,sheet_name,rep_name)
...
def read_ssmx(ssmx_dir, xm_lp_map_mx):
    com_patt = r'^实收款明细表(?P<date>20\d{2}-\d{2}-\d{2} \d{2}_\d{2}_\d{2})\.xlsx$'
    format_date = '%Y-%m-%d %H_%M_%S'
    sheet_name = "实收款明细表"
    rep_name = "实收款明细表"
    df_ssmx = read_vanke_rep(ssmx_dir,com_patt,format_date,sheet_name,rep_name)
...
def _handle_lp(data, xm_lp_map_mx):
    ...
    data['楼盘_合并房号'][data['产品类型']=='车位'] = data['房号'][data['产品类型']=='车位']
