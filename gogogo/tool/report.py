...
# 已草签未转销售系统签约
def ocq_no_sys(df_mxlp,date,index_l=None,is_agg=True):
...
# 网签待转
def wq_wait_sys(df_mxlp,date,index_l=None,is_agg=True):
...
# 已草签待转
def cq_wait_sys(df_mxlp,date,index_l=None,is_agg=True):
    df_mxlp = df_mxlp[df_mxlp['签约日期'] <= date]
...
def rep_map(data_dic,today,index_ls,save_dir=None):
    ...
    rg_qy_l.append(ocq_no_sys(df_mxlp,today,index_l=index_l))
    rg_qy_l.append(wq_wait_sys(df_mxlp,today,index_l=index_l))
